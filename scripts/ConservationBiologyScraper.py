...
'''Importing Selenium to prevent injection of JS elements into the webpage'''
...
def selenium_middleware(url, status_logger_name):
	'''This function acts as a middleware in the code, before passing it onto the Souper.
	This script removes the JavaScript being injected into the webpage, and facilitates scrapping'''
	selenium_middleware_start_status_key = "Triggering Selenium Chromium"
	status_logger(status_logger_name, selenium_middleware_start_status_key)

	'''Creating the Chrome instance from where the HTML will be retrieved, devoid of JS elements'''
	browser = webdriver.Chrome()
	'''Running the URL in the Chrome instance before extracting elements from it'''
	browser.get(url)
	'''Retrieving HTML source code. Will send this over to the souper'''
